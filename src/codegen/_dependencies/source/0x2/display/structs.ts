...
export function isDisplay(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V23}::display::Display` + '<'); }
...
export function isDisplayCreated(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V23}::display::DisplayCreated` + '<'); }
...
export function isVersionUpdated(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V23}::display::VersionUpdated` + '<'); }
...
export class VersionUpdated<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V23}::display::VersionUpdated`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = VersionUpdated.$typeName; readonly $fullTypeName: `${typeof PKG_V23}::display::VersionUpdated<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = VersionUpdated.$isPhantom;

 readonly id: ToField<ID>; readonly version: ToField<"u16">; readonly fields: ToField<VecMap<String, String>>

 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: VersionUpdatedFields<T>, ) { this.$fullTypeName = composeSuiType( VersionUpdated.$typeName, ...typeArgs ) as `${typeof PKG_V23}::display::VersionUpdated<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;
